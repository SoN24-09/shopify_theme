  ...
  customElements.define(
    ...
    class ProductForm extends HTMLElement {
      constructor() {
        ...
        if(!this.matches('.product-form')){
          this.variants = JSON.parse(this.querySelector('[type="application/json"]').innerHTML)
        }
        if(this.closest('.cart-area-wrap')){
          var button = this.closest('.cart-area-wrap').querySelector('.toggle-cart')
...
document.querySelectorAll('.dropdown-item').forEach((item) => {
  item.addEventListener('click', function () {
    ...
    const selector = dropdownMenu.nextElementSibling;
    ...
    if(this.matches('.color')){
      const oldSpan = dropdownButton.querySelector('span');
...
document.addEventListener('click', function (event) {
  ...
  const selectedOption = event.target.closest('.variant-selector__option');
  if (selectedOption) {
    const dropdown = selectedOption.closest('.variant-selector');
    const buttonText = dropdown.querySelector('.variant-selector__button-text');
    const selectedText = selectedOption.textContent.trim();
    
    const colorElement = selectedOption.querySelector('.variant-selector__color');
    const selectedColor = colorElement ? colorElement.getAttribute('data-color-value') : '';

    buttonText.innerHTML = `
      ${selectedColor ? `<span class="variant-selector__color" style="background-color: ${selectedColor}; width: 16px; height: 16px; display: inline-block; border-radius: 50%; margin-right: 5px;"></span>` : ''}
      <span data-option-text>${selectedText}</span>
    `; 
  }
